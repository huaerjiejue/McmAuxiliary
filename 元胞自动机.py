"""
这是简单的元胞自动机的实现，具体的文章中还有森林火灾，蒲公英生成，病毒传播等模拟，
一个简单的元胞自动机的代码实现主要步骤就是：
1. 初始化网格，可以是随机的，也可以是特定的图案，比如滑翔机。
2. 根据规则更新网格，这里是 Conway 生命游戏的规则。
3. 用 matplotlib 动画显示网格的变化。
"""
import sys, argparse
import numpy as np
...
def randomGrid(N):
    ...
    return np.random.choice(vals, N * N, p=[0.2, 0.8]).reshape(
        N, N
    )  # 采用随机的初始状态
...
def main():
    ...
    # add arguments
    parser.add_argument(
        "--grid-size", dest="N", required=False
    )  # 定义了可选参数，指定模拟网格大小N
    parser.add_argument(
        "--mov-file", dest="movfile", required=False
    )  # 指定保存.mov 文件的名称
    parser.add_argument(
    ...
    img = ax.imshow(
        grid, cmap=cmap, interpolation="nearest"
    )  # 用plt.show()方法将这个矩阵的值显示为图像，并给 interpolation 选项传入'nearest'值，以得到尖锐的边缘（否则是模糊的）
