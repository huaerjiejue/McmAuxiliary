...
#     num_plots = 4 if plot_soc else 3
...
#     if plot_soc:
#         axs[3].plot(data_df[time_col], data_df[soc_col], label='SoC')
#
#     for ax in axs:
#         ax.legend()
...
def generate_and_save_plot(data_df: pd.DataFrame, save_file_path: str, fig_title: str = '',
    ...
    # 电压图
    axs[0].plot(data_df[time_col], data_df[voltage_col],
                color=colors['voltage'], linewidth=1.5, alpha=0.8)
    axs[0].set_ylabel('Voltage (V)', fontsize=11, color='#2C2C2C')
    axs[0].tick_params(labelsize=9, colors='#2C2C2C')
    ...
    # 电流图
    axs[1].plot(data_df[time_col], data_df[current_col],
                color=colors['current'], linewidth=1.5, alpha=0.8)
    axs[1].set_ylabel('Current (A)', fontsize=11, color='#2C2C2C')
    axs[1].tick_params(labelsize=9, colors='#2C2C2C')
    ...
    # 温度图
    axs[2].plot(data_df[time_col], data_df[temperature_col],
                color=colors['temperature'], linewidth=1.5, alpha=0.8)
    axs[2].set_ylabel('Temperature (°C)', fontsize=11, color='#2C2C2C')
    axs[2].tick_params(labelsize=9, colors='#2C2C2C')
    axs[2].grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
    axs[2].set_facecolor('#FAFAFA')
    axs[2].spines['top'].set_visible(False)
    axs[2].spines['right'].set_visible(False)

    # SoC图
    if plot_soc:
        axs[3].plot(data_df[time_col], data_df[soc_col],
                    color=colors['soc'], linewidth=1.5, alpha=0.8)
        axs[3].set_ylabel('State of Charge (%)', fontsize=11, color='#2C2C2C')
        axs[3].set_xlabel('Time', fontsize=11, color='#2C2C2C')
        axs[3].tick_params(labelsize=9, colors='#2C2C2C')
        axs[3].grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
        axs[3].set_facecolor('#FAFAFA')
        axs[3].spines['top'].set_visible(False)
        axs[3].spines['right'].set_visible(False)
    else:
        # 如果没有SoC图，给最后一个子图添加x轴标签
        axs[2].set_xlabel('Time(min)', fontsize=11, color='#2C2C2C')

    # 调整布局
    plt.tight_layout()

    # 保存图片，使用更高的DPI以获得更好的质量
    fig.savefig(save_file_path, format='pdf', dpi=900, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    ...
    try:
        ...
        if c20_file:
            ...
            for index, row in df.iterrows():
                ...
                df.at[index, 'Cumulative_Capacity_Ah'] = df.at[
                                                             index - 1, 'Cumulative_Capacity_Ah'] + cumulative_capacity_change
